...
class chart(QChart):
    ...
    def wheelEvent(self, event: PySide6.QtWidgets.QGraphicsSceneWheelEvent) -> None:
        val = pow(0.999, event.delta())
        plotArea = self.plotArea()
        plotAreaCenter = plotArea.center()
        plotArea.setWidth(plotArea.width() * val)
        plotArea.setHeight(plotArea.height() * val)
        newCenterPoint = (2 * plotAreaCenter - event.pos() - (plotAreaCenter - event.pos()) / val)
        plotArea.moveCenter(newCenterPoint)
        self.zoomIn(plotArea)
